...
module {
  aie.device(npu2_1col) {
    ...
    } {keep_pkt_header = true, ctrl_pkt_flow = true}
    aie.packet_flow(0x5) {                             // reply: shim ctrl port -> core
      aie.packet_source<%shim, TileControl : 0>
      aie.packet_dest<%core, DMA : 1>
    } {keep_pkt_header = true, ctrl_pkt_flow = true}
    aie.shim_dma_allocation @slab_in (%shim, MM2S, 0)  // gives the shim a real BD0
    ...
    aie.core(%core) {
      ...
      %h_read = arith.constant 0x0573ff30 : i32   // read 4 words at PL-module 0x3FF30
