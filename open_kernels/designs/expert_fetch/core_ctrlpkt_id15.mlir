...
module {
  aie.device(npu2_1col) {
    %shim = aie.tile(0, 0) {controller_id = #aie.packet_info<pkt_type = 0, pkt_id = 15>}
    %core = aie.tile(0, 2)
    ...
    aie.packet_flow(15) {                       // control: core -> shim ctrl port (overlay-style: id 15, priority, keep header)
    ...
    aie.core(%core) {
      ...
      %h_push = arith.constant 0x0001D214 : i32
      %v_push = arith.constant 0x80000000 : i32   // token issue | BD 0, as the compiler itself writes
    ...
    ^ctrl:
      ...
      aie.dma_bd(%ctrl_buf : memref<8xi32> offset = 0 len = 2) {packet = #aie.packet_info<pkt_id = 15, pkt_type = 0>}
