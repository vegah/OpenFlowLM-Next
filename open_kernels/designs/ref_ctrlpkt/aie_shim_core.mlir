...
module {
  aie.device(npu2_1col) {
    ...
    %ctrl_buf = aie.buffer(%tile_0_2) {sym_name = "ctrl_buf"} : memref<8xi32>
    %ctrl_empty = aie.lock(%tile_0_2, 6) {init = 1 : i32, sym_name = "ctrl_empty"}
    %ctrl_full  = aie.lock(%tile_0_2, 7) {init = 0 : i32, sym_name = "ctrl_full"}
    ...
    %core_0_2 = aie.core(%tile_0_2) {
      ...
      // core-sourced control packet: READ 4 words at shim 0x1D000 (BD0), reply stream id 2
      %h_rd = arith.constant 0x8271d000 : i32
      %z = arith.constant 0 : i32
      %c1_ulc = arith.constant 1 : i32
      aie.use_lock(%ctrl_empty, AcquireGreaterEqual, %c1_ulc)
      memref.store %h_rd, %ctrl_buf[%c0] : memref<8xi32>
      memref.store %z, %ctrl_buf[%c1] : memref<8xi32>
      aie.use_lock(%ctrl_full, Release, %c1_ulc)
      ...
      scf.for %arg0 = %c0 to %c4294967295 step %c1 {
        ...
        aie.use_lock(%output_lock5, AcquireGreaterEqual, %c1_ul5)
        scf.for %arg1 = %c0 to %c8 step %c1 {
            %1 = memref.load %input_buffer[%arg1] : memref<8xi32>
        ...
        aie.use_lock(%output_lock4, Release, %c1_ul6)
      }
      aie.end
    }

    %mem_0_2 = aie.mem(%tile_0_2) {
      %0 = aie.dma_start(MM2S, 0, ^bb1, ^bbc)
    ^bbc:
      %m1 = aie.dma_start(MM2S, 1, ^bbc1, ^bb2)
    ^bbc1:
      %c1_ulc7 = arith.constant 1 : i32
      aie.use_lock(%ctrl_full, AcquireGreaterEqual, %c1_ulc7)
      aie.dma_bd(%ctrl_buf : memref<8xi32> offset = 0 len = 2) {packet = #aie.packet_info<pkt_id = 5, pkt_type = 0>}
      %c1_ulc8 = arith.constant 1 : i32
      aie.use_lock(%ctrl_empty, Release, %c1_ulc8)
      aie.next_bd ^bbc1
    ^bb1:  // 2 preds: ^bb0, ^bb2
      %c1_ul7 = arith.constant 1 : i32
      aie.use_lock(%output_lock4, AcquireGreaterEqual, %c1_ul7)
      aie.dma_bd(%output_buffer : memref<8xi32> offset = 0 len = 8) {packet = #aie.packet_info<pkt_id = 3, pkt_type = 0>}
      %c1_ul8 = arith.constant 1 : i32
      aie.use_lock(%output_lock5, Release, %c1_ul8)
      aie.next_bd ^bb1
    ^bb2:
      aie.end
    }
    ...
    aie.runtime_sequence @seq(%arg0: memref<8xi32>, %arg1: memref<8xi32>, %arg2: memref<8xi32>) {
      ...
      // patch bd0 length back to 2 and address for packet 3 (a 2-word write), push, wait
      %cst_npu_len2a = arith.constant 0x1d000 : i32
      %cst_npu_len2b = arith.constant 2 : i32
      aiex.npu.write32(%cst_npu_len2a, %cst_npu_len2b) {column = 0 : i32, row = 0 : i32} : i32, i32
